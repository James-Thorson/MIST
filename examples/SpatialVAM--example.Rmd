---
title: Example script for SpatialVAM for spatio-temporal analysis of species interactions
  data
...
# Overview
This tutorial will walk through a simple example of how to use `SpatialVAM` for estimating species interactions.
...
devtools::install_github("james-thorson/SpatialVAM") 
```
...
If you have further questions after reading this tutorial, please explore the [GitHub repo](https://github.com/james-thorson/SpatialVAM/#description) mainpage, wiki, and glossary.  Also please explore the R help files, e.g., e.g., `?Data_Fn` for explanation of data inputs, or `?Param_Fn` for explanation of parameters.
...
## How to cite SpatialVAM

`SpatialVAM` has involved many publications for developing individual features.  If using `SpatialVAM`, please read and cite:

```{r citation, tidy=TRUE, width=70, width.cutoff=70}
citation("SpatialVAM")
```

and also browse the [GitHub list](https://github.com/james-thorson/SpatialVAM/#description-of-package) of papers.
...
DateFile = paste0(getwd(),'/SpatialVAM_output/')
  dir.create(DateFile)
```
...
```
![Spatial extent and location of knots](SpatialVAM_output/Data_and_knots.png) 

![Spatial distribution of catch-rate data](SpatialVAM_output/Data_by_year.png) 
...
```
![Density maps for each year for arrowtooth flounder](SpatialVAM_output/Dens--Atheresthes_stomias.png) 

![Density maps for each year for Alaska pollock](SpatialVAM_output/Dens--Gadus_chalcogrammus.png) 
...
```
![Index of abundance plus/minus 1 standard error](SpatialVAM_output/Index.png) 
