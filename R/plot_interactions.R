  ...
  if( !is.null(plotdir)) png( file=paste0(plotdir,"eigen_B_pp.png"), width=3*Ncol, height=3*Nrow, units="in", res=res)
    ...
    for(p in 1:TmbData$n_p){
      if( isComplexTF==TRUE ){
        ...
        legend( "top", bty="n", legend=paste0(ThorsonUtilities::FormatC(Re(Eigen$values[p]),3)," ",ifelse(sign(Im(Eigen$values[p]))<0,"-","+")," ",ThorsonUtilities::FormatC(abs(Im(Eigen$values[p])),3),"i"), cex=1.2 )
      }else{
        ...
        legend( "top", bty="n", legend=ThorsonUtilities::FormatC(Eigen$values[p],3), cex=1.2 )
      }
