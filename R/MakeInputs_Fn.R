...
MakeInputs_Fn = function( Version, options_vec, obsmodel_p=NULL, loc_x, a_x, data_frame, n_cointegrate=NULL, n_factors=1, use_REML=FALSE, independentTF=FALSE, estimate_phi=TRUE ){
  ...
  if( is.null(n_cointegrate)) n_cointegrate = length(unique(data_frame[,'spp']))
  ...
  # Spatial
  if( Version%in%c("spatial_vam_v8","spatial_vam_v7","spatial_vam_v6","spatial_vam_v5","spatial_vam_v4","spatial_vam_v3","spatial_vam_v2","spatial_vam_v1")){
    # Data
    ...
    if( Version%in%c("spatial_vam_v8","spatial_vam_v7","spatial_vam_v6","spatial_vam_v5")) Random = c(Random, "delta_i")
    if(use_REML==TRUE) Random = c(Random, "alpha_p", "phi_p")  # , "B_pp"
    ...
    # Identifiability restrictions on Beta_pr
    if( "Beta_pr" %in% names(Params) ){
