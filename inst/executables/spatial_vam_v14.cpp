...
Type objective_function<Type>::operator() ()
{
  ...
  for(int p=0; p<n_p; p++){
    ...
    if( Options_vec(4)==0 ) Q_spatial = Q_spde(spde, exp(logkappa_z(p)), H);
    ...
    if( Options_vec(4)==1 ) Q_spatial = exp(2*logtauA_p(p)) * (M0*pow(1+exp(logkappa_z(p)*2),2) + M1*(1+exp(logkappa_z(p)*2))*(-exp(logkappa_z(p))) + M2*exp(logkappa_z(p)*2));
  ...
  for(int t=0; t<n_t; t++){
    ...
    if( Options_vec(2)==0 ){
      if( Options_vec(4)==0 ) Q_spatiotemporal = Q_spde(spde, exp(logkappa_z(n_p)), H);
      // FOR EXPLORATION SEE: C:\Users\James.Thorson\Desktop\Project_git\2016_Spatio-temporal_models\Week 6 -- 2D spatial models\Lecture\autoregressive_grid_V1.cpp
      if( Options_vec(4)==1 ) Q_spatiotemporal = exp(2*logtauE_p(0)) * (M0*pow(1+exp(logkappa_z(n_p)*2),2) + M1*(1+exp(logkappa_z(n_p)*2))*(-exp(logkappa_z(n_p))) + M2*exp(logkappa_z(n_p)*2));
