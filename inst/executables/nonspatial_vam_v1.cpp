...
Type objective_function<Type>::operator() ()
{
  ...
  // Slot 0 -- distribution of data: 0=Poisson; 1=Lognormal; 2=Zero-inflated lognormal; 3=lognormal-Poisson; 4=Normal
  ...
  for(int i=0; i<n_i; i++){
    ...
    if( !isNA(c_i(i)) ){                
      ...
      if(Options_vec(0)==1) jnll_comp(2) -= dlognorm( c_i(i), logchat_i(i), sqrt( exp(2*logsigma_pz(p_i(i),0)) + pow(se_log_c_i(i),2) ), true );
      if(Options_vec(0)==2){
      ...
      // Not implemented: if(Options_vec(0)==3) 
      if(Options_vec(0)==4) jnll_comp(2) -= dnorm( c_i(i), exp(logchat_i(i)), exp(logsigma_pz(p_i(i),0)), true );
    }
  }
