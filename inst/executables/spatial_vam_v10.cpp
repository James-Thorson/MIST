...
Type objective_function<Type>::operator() ()
{
  ...
  if( Options_vec(0)==2 ){
    ...
    for(int p=0; p<n_p; p++){
    for(int r=0; r<n_r; r++){
      if(p==0) Alpha_pp(p,r) = 1;
      if(p>=1) Alpha_pp(p,r) = Alpha_pr(p,r);
    ...
    L_pp.setZero();
    for(int r=0; r<n_r; r++) L_pp(r,r) = Alpha_pr(0,r);
    // Build B_pp
  ...
  //vector<Type> MargSigmaA_p(n_p);    //logtauA_p
  //for(int p=0; p<n_p; p++) MargSigmaA_p(p) = pow(4*M_PI,0.5) / exp(logtauA_p(p)) / exp(logkappa);
  vector<Type> logtauA_p(n_p);
  for(int p=0; p<n_p; p++) logtauA_p(p) = 0.5*log(4*M_PI) - logMargSigmaA_p(p) - logkappa;
  vector<Type> MargSigmaA_p = exp( logMargSigmaA_p );
