...
Type objective_function<Type>::operator() ()
{
  ...
  for (int i=0; i<Y.size(); i++){
    if( !NAind(i) ){                
      ...
      if(Options_vec(0)==0) jnll_comp(2) -= dpois( Y(i), exp( log_Dji(s_i(i),t_i(i)) ), true );
      if(Options_vec(0)==1) jnll_comp(2) -= dlognorm( Y(i), log_Dji(s_i(i),t_i(i)), exp(logsigma_z(0)), true );
      if(Options_vec(0)==2){
        encounterprob = ( 1.0 - exp(-1 * exp(log_Dji(s_i(i),t_i(i))) * exp(logsigma_z(1))) );
        log_notencounterprob = -1 * exp(log_Dji(s_i(i),t_i(i))) * exp(logsigma_z(1));
        jnll_comp(2) -= dzinflognorm( Y(i), log_Dji(s_i(i),t_i(i))-log(encounterprob), encounterprob, log_notencounterprob, exp(logsigma_z(0)), true);
      }
    }
  }
  jnll = jnll_comp.sum();
